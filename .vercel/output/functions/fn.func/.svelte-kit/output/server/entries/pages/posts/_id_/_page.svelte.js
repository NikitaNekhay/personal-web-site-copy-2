import { c as create_ssr_component, s as subscribe, f as createEventDispatcher, v as validate_component, e as escape, h as each, b as add_attribute } from "../../../../chunks/index.js";
...
${`${post == void 0 ? `${validate_component(NoPosts, "NoPosts").$$render($$result, {}, {}, {})}` : `<div class="container_slider mt-56"><div class="slider svelte-1bnclxd"><div class="slider-line svelte-1bnclxd" style="${"transform: translateX(" + escape(offset, true) + "px);"}">${post.images ? `${each(post.images, (imag) => {
    ...
    <div class="buttons content-center items-center justify-center text-center"><button class="slider-back transition duration-200 hover:text-orange-0">← ${escape($t("BACK"))}</button>
      <button class="slider-next transition duration-200 hover:text-orange-0">${escape($t("NEXT"))} →</button></div></div>

  <div class="bore mt-4 content-center items-center justify-center text-center"><h1>${escape(post.title)}</h1>
    ...
    <a href="${"mailto:" + escape(post.authorEmail, true)}" class="transition duration-200 hover:text-blue-0">${escape($t("SEND EMAIL TO AUTHOR"))}</a></div>`}`}`;
});
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$unsubscribe_page();
  return `${$$result.head += `<!-- HEAD_svelte-mdmfr5_START -->${$$result.title = `<title>${escape("Page of the post")}</title>`, ""}<meta name="single blog page" content="Detailed blog post"><!-- HEAD_svelte-mdmfr5_END -->`, ""}


${isLoading ? `${validate_component(LoadingSpinner, "LoadingSpinner").$$render($$result, {}, {}, {})}` : `${validate_component(PostDetail, "PostDetail").$$render($$result, { post }, {}, {})}`}
`;
});
export {
  Page as default
