...
<div>
  ...
  >
    ...
    />
    {#if innerWidth < 550 && innerWidth > 414}
      <span class="relative block md:flex lg:flex border border-current bg-white px-4 py-3 ">
        <img class="" alt="setting" src={SvgPath} />
      </span>
    {:else}
      <span class="relative block md:flex lg:flex  border border-current bg-white px-6 py-3 ">
        <img class=" " alt="setting" src={SvgPath} />
      </span>
    {/if}
  </div>
